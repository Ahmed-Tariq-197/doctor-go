...
import {
  Star, MapPin, DollarSign, Users, Clock, ArrowLeft, Calendar, Loader2, CheckCircle,
} from 'lucide-react';
...
const DoctorProfilePage: React.FC = () => {
  ...
  const handleJoinQueue = async () => {
    if (!user) {
      toast({ title: 'Login required', description: 'Please login to join the queue.', variant: 'destructive' });
      navigate('/auth');
      return;
    }
    if (!doctor) return;
    ...
    try {
      ...
      if (response.success) {
        toast({ title: 'Joined queue!', description: `You are now in line.` });
        loadQueue(doctor.id);
        loadDoctor(doctor.id);
      }
    } catch (error) {
      toast({ title: 'Error', description: 'Failed to join queue.', variant: 'destructive' });
    } finally {
  ...
  const handleBookAppointment = async () => {
    ...
    try {
      ...
      if (appointmentResponse.success && appointmentResponse.data) {
        const paymentResponse = await processPayment(appointmentResponse.data.id, doctor.cost);
          ...
          <div className="container mx-auto max-w-lg">
            <Card>
              <CardContent className="pt-8 text-center">
                ...
                <p className="text-muted-foreground mb-8">Your appointment has been booked successfully.</p>
                ...
                <div className="flex gap-4">
                  <Button variant="outline" className="flex-1" onClick={() => navigate('/appointments')}>View Appointments</Button>
                ...
                <CardContent className="pt-6">
                  <div className="flex flex-col md:flex-row gap-6">
                    ...
                    <div className="flex-1 text-center md:text-left">
                      <h1 className="text-2xl md:text-3xl font-bold text-foreground mb-2">{doctor.name}</h1>
                      <Badge variant="secondary" className="text-sm mb-4">{doctor.specialty}</Badge>
                  ...
                  <div className="mt-6 pt-6 border-t border-border">
                    ...
                    <div className="mt-4">
                      <MapPlaceholder doctors={[doctor]} height="200px" />
                    </div>
                  </div>
                </CardContent>
              </Card>
              <AppointmentSlots slots={doctor.availableSlots} selectedSlot={selectedSlot} onSelectSlot={setSelectedSlot} />
            </div>
            ...
            <div className="space-y-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <Button className="w-full" disabled={!selectedSlot || isBooking} onClick={handleBookAppointment}>
                    {isBooking && <Loader2 className="h-4 w-4 mr-2 animate-spin" />}
                    {selectedSlot ? `Book for $${doctor.cost}` : 'Select a time slot'}
                  </Button>
                  <div className="relative">
                    <div className="absolute inset-0 flex items-center"><span className="w-full border-t border-border" /></div>
                    <div className="relative flex justify-center text-xs uppercase"><span className="bg-card px-2 text-muted-foreground">or</span></div>
                  </div>
                  <Button variant="outline" className="w-full" disabled={isJoiningQueue} onClick={handleJoinQueue}>
